...
def fun_FA(centers, maxK, max_iter, n_repeats, s_all=None, verbose=False, conjugate_gradient=True):
    ...
    for i in range(1, maxK + 1):
        ...
        for j in range(1, n_repeats + 1):
            ...
            if not np.isfinite(V1tmp).all():
                continue

            # Re-orthonormalize if numerical drift is detected.
            orth_error = np.linalg.norm(np.matmul(V1tmp.T, V1tmp) - np.identity(i), ord='fro')
            if orth_error > 1e-8:
                V1tmp, _ = qr(V1tmp, mode='economic')
                orth_error = np.linalg.norm(np.matmul(V1tmp.T, V1tmp) - np.identity(i), ord='fro')
            assert orth_error < 1e-6

            # Compute the cost after re-orthonormalization.
...
def CGmanopt(X, objective_function, A, **kwargs):
    '''
    Updated for pymanopt v2.0+ compatibility
    '''
    # 定义流形
    manifold = Stiefel(X.shape[0], X.shape[1])

    # 包装 cost 函数，pymanopt v2 需要装饰器或 autograd 包装
    @pymanopt.function.autograd(manifold)
    def cost(X):
        c, _ = objective_function(X, A)
        return c

    # 创建 Problem (verbosity 参数已从 Problem 中移除)
    problem = Problem(manifold=manifold, cost=cost)

    # 创建优化器 (verbosity 参数移动到了这里)
    # logverbosity=0 在旧版中使用，新版使用 verbosity=0
    optimizer = ConjugateGradient(verbosity=0)

    # 运行优化 (.solve() 改为了 .run()，并返回 Result 对象)
    result = optimizer.run(problem, initial_point=X)
...
def square_corrcoeff_full_cost(V, X, grad=True):
    ...
    # Clamp the residual norms to avoid division by zero.
    c0_safe = np.maximum(c0, 1e-12)
    c0c0T = np.matmul(c0_safe, c0_safe.T)
    Fmn = np.square(C - np.matmul(c, c.T)) / c0c0T
    cost = np.sum(Fmn)/2
    ...
    if grad is False:  # skip gradient calc since not needed
        ...
        PF1 = ((C - np.matmul(c, c.T)) / c0c0T).reshape(P, P, 1, 1)
        PF2 = (np.square(C - np.matmul(c, c.T)) / np.square(c0c0T)).reshape(P, P, 1, 1)
        Gmni = - PF1 * C1 * X1
        Gmni += - PF1 * C2 * X2
        Gmni += PF2 * c0_safe.reshape(P, 1, 1, 1) * C2 * X1
        Gmni += PF2 * (c0_safe.T).reshape(1, P, 1, 1) * C1 * X2
