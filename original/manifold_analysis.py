...
def maxproj(t_vec, sD1, sc=1):
    ...
    # Compute all pairwise dot products at once: (n_t, m)
    # Each row i contains dot products of t_vec[:, i] with all manifold points
    projections = t_vec[0:D, :].T @ sD1[0:D, :]

    # Find index of max projection for each t vector
    max_indices = np.argmax(projections, axis=1)

    # Build s0 by selecting manifold points at max_indices and appending sc
    s0 = np.empty((D1, n_t))
    s0[0:D, :] = sD1[0:D, max_indices]
    s0[D, :] = sc

    # Compute projections of full t vectors onto corresponding s0 vectors
    gt = np.sum(t_vec * s0, axis=0)
