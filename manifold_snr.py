...
def extract_manifold_geometry(manifold: torch.Tensor) -> dict:
    ...
    """
    center = manifold.mean(dim=0)
    manifold = manifold - center
    _, radii, subspace = torch.linalg.svd(manifold, full_matrices=False)
    ...
    dimensionality = (sum_r2 ** 2 / sum_r4).item() if sum_r4 > 1e-10 else 1.0
    return {
        ...
        'total_variance': sum_r2.item()  # total spread of the manifold
    }


def compute_center_subspace_overlap(radii: torch.Tensor, subspace: torch.Tensor, inter_class_direction: torch.Tensor) -> float:
    ...
    if total_variance > 0:
        overlap = ((cos_theta_squared * radii_squared).sum() / total_variance).item()
    else:
        overlap = 0.0
    return overlap


def compute_subspace_overlap(radii_a: torch.Tensor, subspace_a: torch.Tensor, radii_b: torch.Tensor, subspace_b: torch.Tensor) -> float:
    ...
    weighted_overlap = torch.einsum('ij,i,j->', cos_phi ** 2, radii_a_squared, radii_b_squared)
    normalization = radii_a_squared.sum() ** 2

    return (weighted_overlap / normalization).item() if normalization > 0 else 0.0


def pairwise_manifold_snr(manifold_0: torch.Tensor, manifold_1: torch.Tensor, m: int = 1) -> dict:
    ...
    raw_distance = center_diff.norm().item()
    ...
    normalized_distance = raw_distance / (avg_variance_0 ** 0.5) if avg_variance_0 > 0 else 0.0
    ...
    noise = noise_variance ** 0.5 if noise_variance > 0 else 1e-10
    snr = 0.5 * signal / noise
